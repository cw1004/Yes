  ...
  function create(seed) {
    ...
    function resize(width, height, ground) {
      ...
      layers = {
        span: span,
        horizon: horizon,
        riverTop: groundY - Math.max(76, H * 0.115),
    ...
    function fillRidge(ctx, pts, offset, bottom, color) {
    ...
    function draw(ctx, opts) {
      ...
      for (var c = 0; c < clouds.length; c++) {
        ...
        if (cx < 0) cx += L.span;
        if (cx > W + cl.w) continue;
        ctx.beginPath();
        ctx.ellipse(cx, cl.y, cl.w * 0.5, cl.h, 0, 0, Math.PI * 2);
        ctx.ellipse(cx + cl.w * 0.28, cl.y + cl.h * 0.25, cl.w * 0.32, cl.h * 0.75, 0, 0, Math.PI * 2);
        ctx.fill();
      }
      ...
      // 산줄기 (먼 산 → 가까운 산)
      fillRidge(ctx, L.far, scroll * 0.06, L.riverTop, p.far);
      snowCaps(ctx, L.far, scroll * 0.06, 'rgba(255,255,255,0.55)');
      fillRidge(ctx, L.near, scroll * 0.12, L.riverTop, p.near);

      // 대기 원근감
      var hz = ctx.createLinearGradient(0, L.horizon - 40, 0, L.riverTop);
      hz.addColorStop(0, 'rgba(255,255,255,0)');
      hz.addColorStop(1, p.haze);
      ctx.fillStyle = hz;
      ctx.fillRect(0, L.horizon - 40, W, L.riverTop - L.horizon + 40);
      ...
      rg.addColorStop(0, p.river);
      rg.addColorStop(0.55, p.sky[1]);
      rg.addColorStop(1, p.river);
