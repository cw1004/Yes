...
## 1.5 클릭 한 번으로 열기 (스튜디오)

터미널 명령을 외우지 않아도 됩니다. 브라우저에서 회차·언어·화면비를 고르고
버튼 한 번으로 영상을 만들 수 있는 실행 화면이 들어 있습니다.

| 운영체제 | 실행 방법 |
|---|---|
| Windows | `START.bat` 더블클릭 |
| macOS | `START.command` 더블클릭 (처음 한 번은 우클릭 → 열기) |
| Linux | 터미널에서 `./start.sh` |

런처가 패키지 설치 → 환경 점검 → 스튜디오 실행까지 알아서 처리하고
`http://127.0.0.1:8500` 이 브라우저에 자동으로 열립니다.
...
- 회차 범위 입력 시 해당 화 소제목이 바로 표시됨
- 내레이션 언어 / 자막 언어 / 화면비 / 동시 처리 수 / 음성 엔진 / 전환 효과 선택
- 실행 로그 실시간 확인, 진행 중 중지
- 완성된 영상 썸네일 목록에서 바로 재생 · 내려받기

내부적으로는 `python -m india2030 make ...` 를 그대로 실행하므로
CLI 로 만든 결과물과 완전히 같습니다. 서버는 `127.0.0.1` 에만 열리고
파일 제공은 `output` 폴더 안으로 제한됩니다.
