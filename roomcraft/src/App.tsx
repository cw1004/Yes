...
export default function App() {
  ...
  const setResetToken = useAuth((s) => s.setResetToken)
  ...
  // 메일의 재설정 링크(?reset=토큰)로 들어온 경우 새 비밀번호 화면을 엽니다.
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-ink-950">
      ...
      {serverReady === false ? (
        <div className="border-b border-amber-brand/25 bg-amber-brand/8 px-4 py-2 text-center text-xs text-amber-brand">
          ...
          <button
            ...
            className="rounded px-1.5 py-1 underline underline-offset-2 hover:text-mist-200"
          >
            로그인
          </button>{' '}
          하면 크레딧이 서버에 저장됩니다.
      ...
      <footer className="mx-auto max-w-[1720px] px-4 pb-8 pt-2 text-xs leading-relaxed text-mist-500">
        <p>
