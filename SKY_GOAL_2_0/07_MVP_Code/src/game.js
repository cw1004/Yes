...
(function () {
  ...
  var header = null;                 // 헤딩 보너스 선수
  var nextHeaderAt = 0;              // 몇 번째 골문 통과 후 등장할지
  ...
  function startRun() {
    ...
    header = null;
    popups = [];
    nextHeaderAt = 4 + Math.floor(Math.random() * 3);
    usedContinue = false;
    elapsed = 0;
    scroll = 0;
    flash = 0;
  ...
  var HEADER_JUMP_T = 0.62;

  function playerScale() {
    return Math.max(1, Math.min(1.6, H / 700));
  }

  // 점프 중 머리 중심의 화면상 y 좌표
  function headerHeadY(lift) {
    return (groundY + 2) - (117 + lift) * playerScale();
  }

  function updateHeader(dt) {
    // 등장 예약
    if (!header && run.passCount >= nextHeaderAt) {
      header = { x: W + 200, state: 'wait', t: 0, jumpH: 0, lift: 0, hit: false, bob: 0 };
      nextHeaderAt = run.passCount + 7 + Math.floor(Math.random() * 4);
    }
    if (!header) return;

    header.x -= arena.speed * dt;
    header.bob += dt;

    if (header.state === 'wait') {
      // 공이 낮게 다가오면 점프 준비 — 위험을 감수하고 낮게 날아야 만난다
      var near = ball.x > header.x - 170 && ball.x < header.x + 40;
      var low = ball.y > groundY - 215;
      if (near && low && ball.vy > -80) {
        header.state = 'jump';
        header.t = 0;
        header.jumpH = E.clamp(groundY - ball.y - 78, 24, 190);
      }
    } else if (header.state === 'jump') {
      header.t += dt;
      header.lift = header.jumpH * Math.sin(Math.PI * Math.min(1, header.t / HEADER_JUMP_T));

      if (!header.hit) {
        var hy = headerHeadY(header.lift);
        var dx = Math.abs(ball.x - (header.x + 6));
        if (dx < 40 && Math.abs(ball.y - hy) < 36) connectHeader(hy);
      }
      if (header.t >= HEADER_JUMP_T) { header.state = 'done'; header.lift = 0; }
    }

    if (header.x < -90) header = null;
  }

  // 헤딩 성공 — 보너스 점수와 강한 상승
  function connectHeader(headY) {
    header.hit = true;
    var bonus = E.headerBonus(run.combo);
    run.score += bonus;
    run.combo += 1;
    run.headerCount += 1;
    ball.y = headY - 22;
    ball.vy = arena.flap * 1.9;
    ball.spin -= 1.2;
    addSparks(ball.x, ball.y, 20, '255,215,0');
    addSparks(header.x + 6, headY, 10, '255,255,255');
    popups.push({ x: ball.x, y: ball.y - 26, text: 'HEADER! +' + bonus, age: 0, life: 1.1 });
    if (audio) audio.header();
    refreshStage();
    updateHud();
  }
  ...
  function headerPose() {
    var sc = playerScale();
    if (header.state === 'wait') {
      var idle = Math.sin(header.bob * 2.4) * 3;
      return {
        frontLeg: 6, frontBend: 8, backLeg: -8, backBend: 12,
        lean: 2, armF: 14, armB: -12, crouch: idle, headTilt: 0
      };
    }
    var k = Math.min(1, header.t / HEADER_JUMP_T);
    var tuck = Math.sin(Math.PI * k);
    return {
      frontLeg: -18 - tuck * 26, frontBend: 20 + tuck * 46,
      backLeg: 16 + tuck * 34, backBend: 10 + tuck * 30,
      lean: -6 - tuck * 10, armF: -50 - tuck * 40, armB: 40 + tuck * 30,
      crouch: header.lift / sc, headTilt: -14 - tuck * 10
    };
  }

  function drawHeaderPlayer() {
    if (!header || header.x < -60 || header.x > W + 60) return;
    var sc = playerScale();

    // 대기 중에는 "여기로 낮게 오라"는 표시를 보여준다
    if (header.state === 'wait' && header.x < W + 30) {
      var zoneTop = groundY - 215;
      ctx.save();
      ctx.globalAlpha = 0.5 + 0.2 * Math.sin(header.bob * 4);
      ctx.strokeStyle = 'rgba(255,215,0,0.85)';
      ctx.setLineDash([6, 6]);
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.moveTo(header.x - 52, zoneTop);
      ctx.lineTo(header.x + 62, zoneTop);
      ctx.stroke();
      ctx.setLineDash([]);
      ctx.fillStyle = 'rgba(255,215,0,0.09)';
      ctx.fillRect(header.x - 52, zoneTop, 114, groundY - zoneTop);
      ctx.fillStyle = '#ffd75a';
      ctx.font = '700 13px system-ui, sans-serif';
      ctx.textAlign = 'center';
      ctx.fillText('HEADER ↓', header.x + 5, zoneTop - 10);
      ctx.textAlign = 'start';
      ctx.restore();
    }

    var baseY = groundY + 2;
    drawPlayer(header.x, baseY, sc, headerPose(), 1);
  }
  ...
  function showResult(sum) {
    ...
    $('r-line').textContent = '점수 ' + sum.score + ' · 콤보 ' + sum.combo +
      ...
      (sum.headerCount ? ' · 헤딩 ' + sum.headerCount : '');
  ...
  window.SkyGoal = {
    ...
    debug: function () {
      return {
        ...
        header: header ? { x: header.x, state: header.state, hit: header.hit, lift: header.lift } : null,
        nextHeaderAt: nextHeaderAt,
        size: { w: W, h: H, groundY: groundY }
      };
    },
    // 자동화 테스트용: 골문을 비우고 헤딩 선수를 즉시 부른다
    debugClearGates: function () { gates = []; },
    debugSpawnHeader: function () { nextHeaderAt = 0; header = null; },
